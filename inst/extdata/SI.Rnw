...
\makeatother
\subtitle{Twinners in pre-industrial Europe: more babies but fewer births}
\author{I. J. Rickard \and C. Vullioud \and F. Rousset \and E. Postma \and S.
Helle \and V. Lummaa \and R. Kylli \and J. Pettay \and E.
...
However, we fitted parameters on the data, which means that the data tend to be more consistent with the fitted model $\phi(S;\hat{\bth}_\pi)$ than with the data-generating process $\phi(S; \bth)$. This means that, if a GOF test has controlled error rates (uniform p-values) when defined from $\CDF(x;\bth)$, it may be conservative when each sample is assessed against  $\CDF(x;\hat{\bth}_\pi)$ (with parameters and CDF re-estimated on each sample produced by the data-generating process). Whether this problem occurs depends on the relationship between the goodness of fit statistic (more generally denoted $T=\tfn(D)$) and the estimated parameters. A toy example that captures this considers that $T(\data)$ and $\hat{\bth}(\data)$ follow a bivariate Gaussian model
	\begin{equation*}
	(T,\hat{\bth}) \sim \mathcal{N}\left((0,\bth), \begin{pmatrix}
	\sigma^2_t+\beta^2 \sigma^2_{\hat{\bth}}& \beta \sigma^2_{\hat{\bth}}\\
	\beta \sigma^2_{\hat{\bth}}& \sigma^2_{\hat{\bth}}
	\end{pmatrix}\right)
	\end{equation*}
with $\bth$ reduced to a single scalar parameter, and with $\beta$, $\sigma^2_t$ and $\sigma^2_{\hat{\bth}}$ independent of $\bth$. The reason for considering this example is that it leads to a simple linear regression of $T$ to $\hat{\bth}$. Specifically, $T|\hat{\bth}; \bth \sim \mathcal{N}(\beta\hat{\bth},\sigma^2_t)$, so that in the worst case ($\beta=1$), $T$ is centered on $\hat{\bth}$ and then, simulating bootstrap samples from the fitted model (i.e. given $\hat{\bth}$) will yield bootstrapped values $\hat{\bth}^*$, and $T^*$ centered on $\hat{\bth}$ and the p-value deduced by comparing $T$ to the distribution of $T^*$ will tend to be close to 0.5, rather than be uniformly distributed as it should be when we are testing a correct model for the data-generating process.
...
\end{center}
\caption{Relationship between lifetime twinning status, maternal total births and the age at first birth. The age at first birth was significantly higher for twinners with one ($\textrm{AFB}_\textrm{twinners}$ = 31.6 years; 95\% CI: 30.2, 33.0; $\textrm{AFB}_\textrm{non-twinners}$ = 29.3 years; 95\% CI 28.6, 30.0; delay for twinners = 27.5 months; 95\% CI: 12.1, 41.3) or two maternal total births ($\textrm{AFB}_\textrm{twinners}$ = 29.7 years; 95\% CI: 28.5, 31.0; $\textrm{AFB}_\textrm{non-twinners}$ = 28.2 years; 95\% CI: 27.5, 28.7; delay for twinners = 17.9 months; 95\% CI: 5.98, 29.4). Marginal predictions for the age at first reproduction are shown in black with open symbols. Their values are provided by the left y-axis. The distribution of maternal total births for both twinners and non-twinners is shown by the grey lines with filled symbols, with the numerical value given by the log-scaled y-axis on the right of the figure. Note that, here, the 95\% CIs are based on 1000 bootstrap replicates for maternal total births 1 \& 2, but only on 100 replicates for other total births. We did so to reduce the computational load of the analysis for cases which do not require much precision.}
\end{figure}
...
\end{center}
\caption{Representation of the four mechanisms (row 1) used as the basis for 16 simulation scenarios evaluated (all rows). Circles represent the three life history events we considered: parity progression (PP), probability of twinning (T) and interbirth interval (IBI). The rectangles represent the variables potentially shaping these life history events — maternal age and parity at a given birth (Age + Parity above; $\mathtt{poly(cbind(age, parity), best\_order)}$ in models) and whether the last birth was a twin birth or not (Twin above; $\mathtt{twin}$ in models) — as well as a random effect capturing other sources of heterogeneity between mothers (Mum ID above; $\mathtt{maternal\_id}$ in models). Black links represent relationships assumed in all simulation scenarios. Red links represent the given relationship that is considered in a given mechanism in view of accounting for the negative relationship between per-birth twinning probability and maternal total births.}
\end{figure}
...
\end{center}
\caption{Relationship between the per-birth twinning probability and maternal total births. This figure reproduces Main Text Fig. 2 and adds on top of it the same information once families with missing information for birth months (including the entire Norway dataset) are also included in the statistical models. Such new information is illustrated by the dashed regression line with an estimated slope $\beta'$ of -0.0349, as well as with the dotted lines showing the location of the 95\% CI.}
\end{figure}
